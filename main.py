...
# 用户列表，每一个元素是用户对象
# 参考示例：请将下方 {"TODO_你的学号"} 替换为你的真实学号
# 若你的考勤系统密码是默认初始密码 Ahgydx@920，可只填学号
USER_LIST = [
    User(student_Id=259000000)
]
# 单次尝试签到最大尝试次数
MAX_RETRIES = 4
...
WEB_DICT = {
    ...
    "apiLog_api": f"{API_BASE_URL}/flySource-base/apiLog/save?menuTitle=%E6%99%9A%E5%AF%8D%E7%AD%BE%E5%88%B0",
...
UA_LIST = [
    # 微信小程序（Android）真实 UA 形态：miniProgram 标记 + 小程序 wxid
    "Mozilla/5.0 (Linux; Android 15; MIX Fold 4 Build/TK1.240502.001; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/130.0.6723.107 Mobile Safari/537.36 XWEB/1300123 MMWEBSDK/20250401 MMWEBID/7821 MicroMessenger/8.0.61.2660(0x28003157) WeChat/arm64 Weixin NetType/WIFI Language/zh_CN ABI/arm64 miniProgram/wx1a2b3c4d5e6f7g8h",
    "Mozilla/5.0 (Linux; Android 14; LYA-AL10 Build/HUAWEILYA-AL10; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/130.0.6723.107 Mobile Safari/537.36 XWEB/1300123 MMWEBSDK/20250401 MMWEBID/6612 MicroMessenger/8.0.61.2660(0x28003157) WeChat/arm64 Weixin NetType/5G Language/zh_CN ABI/arm64 miniProgram/wx1a2b3c4d5e6f7g8h",
    "Mozilla/5.0 (Linux; Android 13; SM-S9210 Build/TP1A.240610.027; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/130.0.6723.107 Mobile Safari/537.36 XWEB/1300123 MMWEBSDK/20250401 MMWEBID/4471 MicroMessenger/8.0.61.2660(0x28003157) WeChat/arm64 Weixin NetType/WIFI Language/zh_CN ABI/arm64 miniProgram/wx1a2b3c4d5e6f7g8h",
    # 微信小程序（iOS）
    "Mozilla/5.0 (iPhone; CPU iPhone OS 18_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.61(0x18003157) NetType/WIFI Language/zh_CN miniProgram/wx1a2b3c4d5e6f7g8h",
    "Mozilla/5.0 (iPhone; CPU iPhone OS 18_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.61(0x18003157) NetType/5G Language/zh_CN miniProgram/wx1a2b3c4d5e6f7g8h",
]
## *------------------------------------------------------* ##
...
def _get_response_charset(response) -> str:
    try:
        headers = getattr(response, "headers", None)
        if headers:
            content_type = headers.get("Content-Type", "")
            if "charset=" in content_type:
                return content_type.split("charset=", 1)[1].split(";", 1)[0].strip().strip('"').strip("'")
    except Exception:
        pass
    return ""


def _decode_response_bytes(raw: bytes, response=None) -> str:
    charsets = []
    header_charset = _get_response_charset(response)
    if header_charset:
        charsets.append(header_charset)
    charsets.extend(["utf-8", "utf-8-sig", "gb18030", "gbk"])
    tried = set()
...
def http_request_json(user: User, method: str, url: str, *,
    ...
    - params 会拼到URL上（模拟原来的 aiohttp params=... 行为）
    ...
    try:
        with opener.open(req, timeout=timeout) as resp:
            ...
            raw = _decode_response_bytes(resp.read(), resp)
        ...
        try:
            raw = _decode_response_bytes(e.read(), e)
        except Exception:
            raw = ""
